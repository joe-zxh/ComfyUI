...
def estimate_memory(model, noise_shape, conds):
    ...
    memory_required = model.model.memory_required([noise_shape[0] * 2] + list(noise_shape[1:]), cond_shapes=cond_shapes) # 正常来说是pos 和 neg一起推理，所以这里batchsize设为2
    minimum_memory_required = model.model.memory_required([noise_shape[0]] + list(noise_shape[1:]), cond_shapes=cond_shapes_min) # 但如果显存不够，comfyui会让pos和neg分开推理，这个就是minimum_memory。也就是samplers.py里面的calc_cond_batch
...
def _prepare_sampling(model: ModelPatcher, noise_shape, conds, model_options=None):
    ...
    comfy.model_management.load_models_gpu([model] + models, memory_required=memory_required + inference_memory, minimum_memory_required=minimum_memory_required + inference_memory) # 如果有额外的网络，需要在这里在load一下
